...
for (const arrayElement of usersList) {
    ...
    for (const item in arrayElement) {
        ...
        if (typeof arrayElement[item] === "object") {
            for (const item1 in arrayElement[item]) {
    ...
    id.innerText = `id: ${arrayElement.id}
    `
    let name = document.createElement('div');
    name.innerText = `name: ${arrayElement.name}`

    let userName = document.createElement('div');
    userName.innerHTML =`<b>username:</b> ${arrayElement.username}`

    let email = document.createElement('div');
    email.innerHTML = `<b>email:</b> ${arrayElement.email}`

    let address = document.createElement('div');
    address.innerText = 'address:'

        let street = document.createElement('div');
        street.innerText = `street: ${arrayElement.address.street}`
    ...
    div0.append(id, name, userName, email, address, phone, website, company);
